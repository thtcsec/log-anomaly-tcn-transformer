...
\subsection{Mô hình mạng tích chập thời gian TCN}
\label{sec:tcn}
Để mở rộng khả năng xử lý song song và cải thiện độ trễ, chúng tôi tích hợp mô hình mạng tích chập thời gian (TCN) \cite{b19} dưới dạng next-event prediction. TCN sử dụng các lớp tích chập 1D nhân quả và giãn nở (Dilated Causal 1D Convolution) để thu nhận receptive field lớn mà không gặp hiện tượng suy giảm gradient như RNN. 

Phép tích chập nhân quả giãn nở với bộ lọc $f: \{0, \ldots, k-1\} \rightarrow \mathbb{R}$ trên phần tử $s$ của chuỗi đầu vào $x$ được định nghĩa là:
\begin{equation}
F(s) = (x *_d f)(s) = \sum_{j=0}^{k-1} f(j) \cdot x_{s - d \cdot j}
\end{equation}
Trong đó $k$ là kích thước bộ lọc (kernel size), $d$ là hệ số giãn nở (dilation factor), và $s - d \cdot j$ biểu thị các vị trí sự kiện trong quá khứ. Cấu trúc mạng TCN bao gồm các khối block dư (residual block) có cơ chế chặn tràn nhân quả (Chomp1d) để đảm bảo không rò rỉ thông tin tương lai. 

Trong thực nghiệm, mạng TCN có cấu hình tương đồng với DeepLog: embedding size 64, 3 block tích chập với số kênh ẩn 64, dilation tăng dần theo cấp số nhân $[1, 2, 4]$, kernel size 2. Đầu ra của TCN tại thời điểm cuối cùng được đưa qua lớp tuyến tính và softmax để dự báo xác suất sự kiện tiếp theo:
\begin{equation}
\hat{P}(E_i \mid E_{i-g}, \ldots, E_{i-1}) = \text{softmax}(W_{tcn} h^{tcn}_i + b_{tcn})
\end{equation}
Mô hình TCN sử dụng chung tập dữ liệu huấn luyện, chế độ chấm điểm (top-k và NLL threshold) và giao thức lựa chọn ngưỡng percentile 95 như DeepLog để bảo đảm tính nhất quán trong so sánh.
...
Đối với định hướng mạng doanh nghiệp và công nghiệp quy mô lớn, dữ liệu LMS thực tế nên được thu thập trong giai đoạn tiếp theo từ máy chủ web, API gateway, database, hệ thống xác thực và dịch vụ cloud. Do log giáo dục có thể chứa thông tin cá nhân, các trường như user ID, email, IP và session ID cần được ẩn danh trước khi huấn luyện.
...
Cột HDFS của Bảng \ref{tab_cross_dataset} cho ba quan sát. Thứ nhất, DeepLog ở chế độ top-k đạt precision rất cao ($0.9817 \pm 0.0188$) nhưng recall hạn chế ($0.3013 \pm 0.0167$): khi LSTM cảnh báo thì gần như chắc chắn đúng, nhưng phần lớn anomaly vẫn lọt qua vì $E_i$ vẫn nằm trong tốp 9. Thứ hai, khi DeepLog đổi sang chế độ NLL threshold (cùng giao thức percentile 95 với các baseline khác), F1 giảm về $0.3037 \pm 0.0165$, xấp xỉ Transformer val-tuned; tức là chiến lược xếp hạng anomaly đóng vai trò quyết định, không chỉ kiến trúc. Thứ ba, trên HDFS cả Transformer mini, DeepLog lẫn TCN đều chưa vượt PCA, gợi ý rằng trong benchmark có vocabulary nhỏ và chuỗi ngắn, lợi thế ngữ cảnh chưa xuất hiện; đây cũng là lý do bài báo bổ sung đánh giá chéo BGL ở §\ref{sec:bgl}.
...
\begin{table*}[!t]
\caption{So sánh chéo các nhánh chấm điểm trên cùng pipeline Drain3 cho HDFS và BGL, trung bình $\pm$ độ lệch chuẩn trên 5 random seed (21, 42, 84, 123, 777). HDFS baseline dùng 500k dòng/36.305 chuỗi BlockId; HDFS sequence model dùng 200k dòng/15.639 chuỗi; BGL dùng 500k dòng/5.000 cửa sổ $W{=}100$.}
\centering
\scriptsize
...
Domain shift LMS & HDFS không chứa hành vi nghiệp vụ như đăng nhập, truy cập dữ liệu, lỗi phân tán & Thu thập log hạ tầng mạng và đánh giá theo session/user/window \\
\bottomrule
\end{tabular}
...
\subsection{Phân tích ứng dụng trong mạng doanh nghiệp và công nghiệp quy mô lớn}
