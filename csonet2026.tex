...
The main contributions include: (1) an end-to-end pipeline of Drain3 parsing, event sequencing, and anomaly scoring with five alternative branches (PCA/SVD, Isolation Forest, DeepLog, TCN, and Transformer masked event modeling) evaluated across HDFS, BGL, and a production-style institutional web-log dataset HUFLIT-Career; (2) a multi-seed evaluation (5 random seeds) on HDFS with a protocol that prevents test set leakage, along with a grouping strategy study for Block ID, fixed window, and sliding window; (3) a cross-dataset evaluation on the same pipeline; (4) an ablation study for Drain3 threshold, DeepLog top-k, and Transformer mask ratio; (5) an inference latency benchmark, expected error analysis, and qualitative explainability case study for deployment-oriented interpretation.
...
\subsection{Datasets}
The study utilizes HDFS and BGL from the LogHub benchmark \cite{b6,b12}, alongside HUFLIT-Career, a production-style institutional web-log dataset collected from a live university web application server. HDFS contains logs from a Hadoop system and is labeled by Block ID (anomalous if the block contains errors); we evaluate on the first 500,000 log lines. BGL contains logs from a supercomputer system, where anomaly labels are assigned using non-overlapping windows of $W{=}100$ lines. HUFLIT-Career is collected from an online career service. Scanner-probe labels in HUFLIT-Career were assigned based on rule-based security signatures matching known automated scanning User-Agents (e.g., vulnerability scanners, requests/curl scripts), path traversal attempts, and anomalous HTTP status code distributions (such as concentrated 403 Forbidden and 404 Not Found responses). An event sequence is labeled anomalous if it contains at least one log line matching these security signatures. Client IP addresses are utilized strictly for sequence grouping and are not included as features for any model. The statistics of the three datasets after Drain3 parsing are described in Table~\ref{tab_dataset_stats}.
