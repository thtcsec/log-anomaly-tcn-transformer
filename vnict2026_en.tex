...
\textbf{Grouping Strategy Study (RQ1):} To address RQ1, we evaluate the anomaly detection performance on BGL under four grouping configurations: non-overlapping fixed windows of size $W{=}50, 100, 200$, and a sliding window of size $W{=}100$ with stride $50$. Table VI shows the F1-score results for PCA, DeepLog (top-k), and TCN (top-k).

The results in Table VI indicate that the window size $W$ directly impacts detection performance. For PCA, F1-score increases significantly from 0.1353 to 0.4381 as $W$ grows to 200, suggesting that larger window contexts stabilize frequency distributions. Conversely, sequence models (DeepLog, TCN) show robust performance across all window configurations. Crucially, the sliding window configuration ($W{=}100, \text{stride}=50$) yields the highest performance (DeepLog 0.9004, TCN 0.9032) by increasing training sample density and mitigating border-crossing boundary effects.
...
Three observations from Table V: (1) Drain3 processes $\sim$228k logs/sec, which is not a bottleneck; (2) PCA/SVD are the fastest ($<0.2$~ms); IF is 100$\times$ slower due to traversing 200 trees; (3) TCN and DeepLog benefit from GPU execution (latency drops from $\sim$1.7ms to 0.45ms), whereas the small Transformer does not benefit at batch size 1. Training times: Drain3 takes $3.49$~s, PCA $0.74$~s, and Transformer $10.75$~s — a trade-off that needs consideration in real-time monitoring.
...
\textbf{Qualitative Explainability Analysis:} To enhance operational utility, we present a qualitative explainability layer based on next-event prediction mismatches. Table VII illustrates two concrete anomaly cases extracted from HDFS and BGL test sets using TCN. When a sequence is flagged, the system outputs the failed event position, the actual versus top-3 predicted event templates, and a severity score (NLL). For instance, in BGL sequence \texttt{W20200}, the unexpected occurrence of \texttt{E18} (data TLB error interrupt) instead of the highly probable normal transitions (\texttt{E3}, \texttt{E6}, \texttt{E4}) produces a high anomaly score ($12.68$), immediately guiding operators to a virtual memory subsystem fault.
...
\toprule
\textbf{Log Set} & \textbf{Sequence ID} & \textbf{Mismatch Event (Actual)} & \textbf{Top-3 Predicted} & \textbf{Interpretation} \\
\midrule
HDFS & blk\_-68076... & E19: Receiving empty packet... & E5, E3, E4 & block replication/write failure \\
BGL & W20200 & E18: data TLB error interrupt... & E3, E6, E4 & kernel page fault / system warning \\
\bottomrule
\end{tabular}
